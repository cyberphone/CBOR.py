...
def reducedOneTurn(length, value, result, exact):
  if isinstance(value, int): value = float(value)
  if isinstance(result, int): result = float(result)
  ok = length != None
  if not ok: length = 2
  try:
    reduced = CBOR.Float.create_float16(value, exact) if length == 2 else CBOR.Float.create_float32(value, exact)
    assert_true("Should not", ok)
    assert_true("Compare", exact == bool(reduced.get_float64() == value))
...
def oneTurn(statement, access, message=None):
    ...
    if access:
        try:
            ...
            check_exception(e, "never read")
    assert_true(repr(message) + error, not should_fail or (error.find(message) >= 0))
